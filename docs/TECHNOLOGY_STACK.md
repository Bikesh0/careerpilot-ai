...
| **ollama (Python client) 0.4.x** | Local LLM client | `app/ai/llm.py` (`LocalLLM`), `app/ai/ai_engine.py` (`AIEngine`) | Talks to a locally-running Ollama server - no hosted API, no API key, keeps candidate data on the local machine by default | `OLLAMA_HOST`, `OLLAMA_MODEL`, `OLLAMA_TIMEOUT`, `OLLAMA_MAX_TOKENS` env vars (see `docs/AI.md`) | Requires Ollama installed and running locally with a model pulled; without it, generation features return a clear "unavailable" response rather than working |
| **python-docx 1.1.x** | `.docx` document generation and parsing | `app/documents/resume_template.py` (generation), `app/parsers/cv_parser.py` (`.docx` CV parsing) | Standard library for reading/writing Word documents in Python | None | `.docx` only - doesn't handle legacy `.doc` |
| **PyMuPDF 1.24.x** (imported as `fitz`) | PDF text extraction | `app/parsers/cv_parser.py` | Fast, reliable PDF text extraction | None | `CVParser` itself is not currently reachable from any Flask route (see `docs/PRODUCT_VISION.md`) |
